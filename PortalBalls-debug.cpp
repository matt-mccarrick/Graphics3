...
/*	// Default color of world.
	float globalAmbient[] = {0.6, 0.6, 0.6, 1.0};
	
	// assign global light property
...
/*
	...
	*/
    ...
    float materialDiffuse[] = {0.2, 0.2, 1.0, 1.0};
    float materialSpecular[] = {0.6, 0.3, 0.3, 1.0 };
    float materialShininess[] = {32.0};
	float materialEmission[] = {0.2, 0.2, 1.0, 1.0};
		...
		if(balls[i].exists){
			glPushMatrix();
				...
				if(i % 2 == 0){
					...
					//glEnable(GL_LIGHT0);
					//glLightfv(GL_LIGHT0, GL_POSITION, balls[i].pos);
				}
				else{
					glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 216,286, 0, GL_RGB, GL_UNSIGNED_BYTE, orangeball);
					//glEnable(GL_LIGHT1);
					//glLightfv(GL_LIGHT1, GL_POSITION, balls[i].pos);
				}
...
void readTextures()
{
	...
	/** Jared Dickson, 04-25-2011
	 * Load texture for the blue ball.
	 */
	...
	/** Jared Dickson, 04-25-2011
	 * Load texture for the orange ball.
	 */
